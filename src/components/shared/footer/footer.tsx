...
export default function Footer() {
    return (
        <footer className="mt-12 relative min-h-80 max-laptop:min-h-[1000px]">
            ...
            <div className="text-white pt-8 flex max-laptop:flex-col max-laptop:items-center justify-center gap-32 max-laptop:gap-16">
                {
                        ...
                        <ul className="flex flex-col gap-4" key={index + 1}>
                            <span className="font-bold text-xl">{title}</span>
                            {
                                lists.map((list, index) => (
                                    <li key={index + 1} className="font-medium text-sm hover:text-tint-7 cursor-pointer">{list}</li>
                                ))
                            }
                            <div className="flex items-center gap-4">
                                {
                                    icons?.map((icon, index) => (
                                        <Icon
                                            name={icon}
                                            key={index + 1}
                                            className="size-6 hover:text-tint-7 cursor-pointer"
                                        />
                                    ))
                                }
                            </div>
